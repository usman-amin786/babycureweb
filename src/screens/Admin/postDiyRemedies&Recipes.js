...
const PostDiyRemediesRecipes = () => {
  const [imageUrl, setImageUrl] = useState("");
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");

  const doPost = () => {
    axios.post('http://localhost:3000/diyRemediesRecipies', {
      imageUrl,
      title,
      description,
    ...
    <>
      ...
      >
        <div className="container-fluid pb-5 pt-3">
          <h4>Post Diy Remedeies & Recipes</h4>
          ...
          <form >
            ...
            <div className="text-center mt-4">
              <button style={{marginTop:"20px", boxShadow: '1px 2px 9px #000',
    padding: '1em'}} onClick={doPost} type="submit" className="btn btn-primary">
                Post DiyRemedies Recipies
              </button>
            </div>
          </form>
        </div>
...
export default PostDiyRemediesRecipes;
